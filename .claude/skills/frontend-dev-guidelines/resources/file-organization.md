...
Proper file and directory structure for maintainable, scalable frontend code in the application.
...
useAuth.ts
useGridLayout.ts
use-toast.ts         # ShadCN style (kebab-case also acceptable)
```
...
3. **components/ui/** for ShadCN components
4. Use subdirectories: api/, components/, hooks/, helpers/, types/
5. Import aliases for clean imports (@/, ~types, ~components, ~features)
6. Consistent naming: PascalCase components, camelCase utilities
7. Export public API from feature index.ts
